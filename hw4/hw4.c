...
int main( int argc, char **argv )
{
	...
	for ( k = 0; k < 3; k ++)
	{
		...
		/* Step1: Compute Gx, Gy, SGM, find out the maximum and normalize, use max[0], max[1], max[2] to store the maximum value*/
		
			// Step 1-1: Find out the maximum value for Gx, Gy, SGM
			/*
				Hint: Gx at (i, j) =  abs(  image[i-1][j-1] + 2*image[i][j-1] + image[i+1][j-1]
									- image[i-1][j+1] - 2*image[i][j+1] - image[i+1][j+1])
			*/
			// Write your code here. 
		

			// Step 1-2: Compute Gx and normalize it to [0, 255] by converting to float data type
			/*
			...
			*/
			...
			// Step 1-3: Compute Gy and normalize to to [0, 255] by converting to float (similar to step 2)
			// Write your code here. 
		...
		// Step 2: Compute squared gradient magnitude again and normalize to [0, 255] by converting to float
		/*
			dedx = ...
			dedy = ...
		...
		*/
		...
		// need to check histogram in image viewer and select proper threshold, simialr method in assignment 3
